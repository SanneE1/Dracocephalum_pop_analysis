...
```{r 14}
# concusion from above is that we should probably throw seedlings in vegetative stage. update stage variable
data1 <- data %>% 
...
Below are the models tested and the summary of the model with the lowest AICc
```{r 16}


s_all = glmer(survival_t1 ~ log(n_stems_t0) + population + herb_shading_t0 + shrub_shading_t0 + 
                soil_d1 + soil_d2 + soil_d3 + rock + slope + (1|year_t0), family = "binomial", 
              data = data1 %>% 
                filter(stage_t0 != "dead"))

confint(s_all, method = "Wald")

s_model <- glmer(survival_t1 ~ log(n_stems_t0) + population + herb_shading_t0 + shrub_shading_t0 +
                   slope + (1|year_t0), family = "binomial",
                 data = data1 %>% 
                   filter(stage_t0 != "dead"))
...
# Only herb shading (besides n_stems) has CI not overlapping 0
g_all = lmer(log(n_stems_t1) ~ log(n_stems_t0) + population + 
               herb_shading_t0 + shrub_shading_t0 + 
               soil_d1 + soil_d2 + soil_d3 + slope + rock + (1|year_t0), 
             data = data1 %>% 
               filter(n_stems_t1 > 0 & n_stems_t0 > 0))

confint(g_all, method = "Wald")

g_model <- lmer(log(n_stems_t1) ~ log(n_stems_t0) + 
                  herb_shading_t0 + (1|year_t0), 
             data = data1 %>% 
               filter(n_stems_t1 > 0 & n_stems_t0 > 0))
...
fp_all = glmer(flower_p_t0 ~ log(n_stems_t0) + population + 
                 herb_shading_t0 + shrub_shading_t0 + 
                 soil_d1 + soil_d2 + soil_d3 + rock + slope + 
                 (1|year_t0), family = "binomial", 
               data = data1 %>% 
                 filter(n_stems_t0 > 0))

confint(fp_all, method = "Wald")

fp_model <- glmer(flower_p_t0 ~ log(n_stems_t0) + 
                    population + shrub_shading_t0 + 
                 soil_d3 + slope + (1|year_t0), 
                 family = "binomial", 
               data = data1 %>% 
                 filter(n_stems_t0 > 0))
...
ab_all <-  glmer(seed_p_t0 ~ log(n_stems_t0) +
                   population + 
                   herb_shading_t0 + shrub_shading_t0 + 
                   soil_d1 + soil_d2 + soil_d3 + rock + slope + (1|year_t0), 
                 data = fn_data, 
                 family = "binomial")

confint(ab_all, method = "Wald")

## CI not spanning 0: population, shrub_shading, soil_d3
ab_model <-  glmer(seed_p_t0 ~ log(n_stems_t0) +
                   population + shrub_shading_t0 + 
                   soil_d3 + (1|year_t0), 
                 data = fn_data, 
                 family = "binomial")
...
# number of seeds
ns_all <- glmer(est_seed_n_t0 ~ log(n_stems_t0) +
                population + 
                herb_shading_t0 + shrub_shading_t0 + 
                soil_d1 + soil_d2 + soil_d3 + rock + slope + (1|year_t0), 
              data = fn_data %>% 
...
confint(ns_all, method = "Wald")

## CI not spanning 0: population + shrub_shading

sn_model <- glmer(est_seed_n_t0 ~ log(n_stems_t0) + population + 
                    shrub_shading_t0 + (1|year_t0), 
              data = fn_data %>% 
                filter(seed_p_t0 == 1),
              family = Gamma(link = log))
